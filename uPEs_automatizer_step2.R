...
directory<-args[1]   #/home/nostromo/data/pepe/43_UPEFINDER_AGO20/CCLE/""
database<-args[2]   #GOCC, GOBP...
datos<-args[3]   #### CCLE, GTEX ...
outname_chunk<-args[4]

if(datos=="TCGA"){

        file_loaded<-get(load(paste0(directory,"TCGA_c.Rdata")))
        ...
        save(uPE1_enrichedGO, file=paste0(output_dir,"/",name_chunk,"_output.rda"))

}else if(datos=="GTEX"){

        file_loaded<-get(load(paste0(directory,"GTEX_c.Rdata")))
        ...
        file_loaded<-get(load(paste0(directory,"CCLE_c.Rdata")))
