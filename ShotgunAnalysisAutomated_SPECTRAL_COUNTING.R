...
txtFileName <- args[1]
dataset <- args[2]
DBrdaFileName <- args[3]
decoy_id <- args[4] #mn	"D_"
...
if (error == 0)
{
	...
	}else
	{
			...
			dataPSMMat_filterAA_psmFDR <- psmFDR(dataPSMMat, pepScore=pepScore, decoy_id=decoy_id, concat_decoy = concat_decoy, prot_id = prot_id)
			write.table(dataPSMMat_filterAA_psmFDR,file=paste(currentDir, "/results_Peptides_", search_engine_str, "_", dataset, "_dataPSMMat_NO_filterPSM.txt", sep = ""), row.names=F, col.names=T, quote=F, sep="\t")

			dataPSMMat_filterAA_psmFDR_Filter <- dataPSMMat_filterAA_psmFDR[dataPSMMat_filterAA_psmFDR$psmFDR < psmFDRvalue,]
			save(dataPSMMat_filterAA_psmFDR_Filter,file=paste(currentDir, "/results_Peptides_", search_engine_str, "_", dataset, "_dataPSMMat_filterPSM.rda", sep = ""))
			write.table(dataPSMMat_filterAA_psmFDR_Filter,file=paste(currentDir, "/results_Peptides_", search_engine_str, "_", dataset, "_dataPSMMat_filterPSM.txt", sep = ""), row.names=F, col.names=T, quote=F, sep="\t")
