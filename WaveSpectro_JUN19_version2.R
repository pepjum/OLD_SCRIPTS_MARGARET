...
for (i in 1:nrow(df_spectrums_mgf)){
    ...
    for (i in unique(spectra$idx)){
        charge_sel<-spectra$charge[i]
    ...
    list_all_peaks<-paste(spectra$peaks)
    ...
    spectra$peaks<-NULL
    spectra$wavelet_mgf<-wavelets

    #spectra$idx<-seq(1:(nrow(spectra)))

    cat("merge","\n")
    df_results_tmp<-merge(spectra,df_spectrums_selected_for_each_spectrum_filtered, by="idx", all.x=TRUE )
