...
for (i in 1:nrow(df_spectrums_mgf)){
    spec<-as.data.frame(df_spectrums_mgf[i,])
    #cat("processing  ", spec$spectrum, "\n")
    cat(paste(i, ifelse(i %% 100 == 0,"\n"," ")))

    #cat("selecting spectrums for MZ range","\n")
    selecting_spectrums_c<-cmpfun(selecting_spectrums)

    df_spectrums_selected_for_each_spectrum<-ldply(future_apply(spec,1, function(x) selecting_spectrums_c(x,range), future.seed=TRUE ),rbind)

    if (nrow(df_spectrums_selected_for_each_spectrum)==0){
        next
    }

    list_all_peaks<-paste(spec$peaks)
    ...
    spec$peaks<-NULL
    spec$Peaks_mgf<-paste(list_linecoded)

    #cat("merge","\n")
    df_results_tmp<-merge(spec,df_spectrums_selected_for_each_spectrum, by="idx", all.x=TRUE )
...
df_results_tmp2<-ldply(list_df_for_each_spec, rbind)
