...
#dot_product<-c()
#Dot_bias<-c()
df_results_tmp2$D<-NA
df_results_tmp2$DB<-NA
...
for (i in 1:nrow(df_results_tmp2)){
    ...
    df_results_tmp2$D<-sum(as.numeric(paste(df_peaks_mgf_tmp2$Z))*(as.numeric(paste(df_peaks_database_tmp2$Z))))
    #df_tmp_res$pot<-(as.numeric(df_tmp_res$Z)^2)*(as.numeric(df_tmp_res$Z_database)^2)
    df_results_tmp2$DB<-sqrt(sum(as.numeric(as.numeric(df_peaks_mgf_tmp2$Z)^2)*(as.numeric(df_peaks_database_tmp2$Z_database)^2)))/df_results_tmp2$D
...
for (i in unique(df_results_tmp2$idx)){
    ...
    tmp_max1<-tmp[(which(as.numeric(paste(tmp$D)) ==max(as.numeric(paste(tmp$D))))),]
    tmp_max2<-tmp[-c(which(as.numeric(paste(tmp$D)) ==max(as.numeric(paste(tmp$D)))),]
    tmp_max3<-tmp_max2[(which(as.numeric(paste(tmp_max2$D))) ==max(as.numeric(paste(tmp_max2$D)))),]
...
save(df_results_tmp3, file =paste0(new_name,".Rdata"))
write.table(df_results_tmp3, file=paste0(new_name,".txt"), row.names=F, col.names=F, quote = FALSE, sep="\t")
