...
# }
df_spectrums_selected_for_each_spectrum_filtered<-df_spectrums_selected_for_each_spectrum
...
if (method == "dot"){
    ...
    df_results_tmp2<-df_results_tmp2[with(df_results_tmp2, order(idx,as.numeric(-D))),]
    ...
    new_name<-paste(substr(mgf, 1, nchar(mgf) - 3), "_results_wd_dot_product", sep="")
    ...
    library(stylo)
    Dist_cosine_wav_c<-cmpfun(Dist_cosine_wav)
    dist_cosine<-future_apply(df_results_tmp2,1, FUN = function(x) Dist_cosine_wav_c(x), future.seed=TRUE)
    df_results_tmp2$Dist_cosines<-paste(dist_cosine)
    df_results_tmp2<-df_results_tmp2[with(df_results_tmp2, order(idx,as.numeric(Dist_cosines))),]
    df_results_tmp2$wavelet<-NULL
    df_results_tmp2$wavelet_mgf<-NULL
