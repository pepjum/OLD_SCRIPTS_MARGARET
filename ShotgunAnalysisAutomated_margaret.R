			...
			dataPSMMat <- data.frame()
				for (j in 1:nrow(experimentData))
				{
					if (j == 1)
					{
						...
						{
							dataPSMMat <-  data.frame("file" = paste(experimentData[1,1]), parseXTandemStandardOutput_v_NEW(paste(experimentData[1,1])), "database" = paste(experimentData[1,2]), "sample" = paste(experimentData[1,3]), "fraction" = paste(experimentData[1,4]), "datfile" = paste(experimentData[1,5]))
						}
